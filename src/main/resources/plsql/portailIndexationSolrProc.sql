...
l_host_name        VARCHAR2 (128)   := 'xxxxxxxxxxxxxxxxxxx.fr'; --web server url
l_port             VARCHAR2 (128)   := '8100';
req_length binary_integer;
bufferY   CLOB;
amount  pls_integer:=4000;
offset  pls_integer:=1;
l_contexte_request VARCHAR2 (128)   := 'portail';
l_doc_request      CLOB;
l_content  CLOB;
url varchar2(2000);
...
BEGIN
  ...
  --select TO_CHAR(dateparam, 'dd/mm/yy') into dateinsertion from dual;
  --print_clob_to_output (l_doc_request);
  l_content := '{"$contexte$":"'||l_contexte_request||'","$iddoc$":"'||iddoc||'","$doc$":"'||l_doc_request||'","$texte$":"'||texteparam||'","$dateinsertion$":"'||dateparam||'","$"}';
...
BEGIN
    ...
    l_response := apex_web_service.make_rest_request(
        p_url => 'http://serveur:port/indexationsolr/GetIndexationSolrPortail?iddoc='||iddoc||'&dateInsertion='||dateparam||'&contexte=portail',
        p_http_method => 'GET');
...
l_raw_data         RAW (512);
l_clob_response    CLOB;
l_host_name        VARCHAR2 (128)   := 'xxxxxxxxxxx.fr'; --Web Server URL
