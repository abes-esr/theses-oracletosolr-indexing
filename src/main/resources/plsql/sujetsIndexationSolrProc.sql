...
l_host_name        VARCHAR2 (128)   := 'xxxxxxxxxxxxxxxx.fr'; --Web Server URL
...
BEGIN
    ...
    l_response := apex_web_service.make_rest_request(
        p_url => 'http://serveur:port/indexationsolr/GetIndexationSolr?iddoc='||iddoc||'&contexte=sujets',
        p_http_method => 'GET');
...
l_raw_data         RAW (512);
l_clob_response    CLOB;
l_host_name        VARCHAR2 (128)   := 'xxxxxxxxxxxx.fr'; --Web Server URL
l_port             VARCHAR2 (128)   := '80'; ----Web Server port
req_length binary_integer;
bufferY   CLOB;
amount  pls_integer:=4000;
offset  pls_integer:=1;
